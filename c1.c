...
//打印
void printList( struct Node* List1)
{
	struct Node* p = List1->next;
	printf("姓名\t\t学号\t\t电话\t\tQQ号\n");
	while (p)
	{
		printf("%s\t%s\t%s\t%s\n", p->data.name, p->data.num, p->data.tele, p->data.qq);
		p = p->next;
...
struct Node* changeNode(struct Node* List1, char* posData,struct  student info1, struct student *p1)
{
	...
	while (1)
	{
		if ((change_choice != 1 && change_choice != 2 && change_choice != 3 && change_choice != 4 && change_choice != 5 && k == 1) || (k != 1))
		{
			if (k != 1)
			{
				getchar();
								...
								case 3:
									while (1)
									{
										...
										case 1:
											...
											system("cls");
											printf("\n请输入新成员的个人信息：\n姓名\t\t学号\t\t电话\t\tQQ\n");
											scanf("%s\t%s\t%s\t%s", info1.name, &info1.num, &info1.tele, &info1.qq);
